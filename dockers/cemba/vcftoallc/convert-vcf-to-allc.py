...
def main():
    ...
    parser.add_argument(
      "--input_vcf", "-i", dest="input_vcf", default=None, help="input VCF"
    )
    parser.add_argument(
      "--output_allc", "-o", dest="output_allc", default=None,
      help="output ALLC"
    )
    ...
    writeBinary = False
    if args.input_vcf:
        with gzip.open(args.input_vcf, "rt") if args.input_vcf.endswith(
            ".gz"
        ) else open(args.input_vcf, "r") as vcf_file:
            with gzip.open(args.output_allc, "wb") if args.output_allc.endswith(
                ".gz"
