...
#QUAY_URL="quay.io/broadinstitute/gotc-prod-snaptools" # Update and uncomment push block below after setting up quay repo

SNAPTOOLS_VERSION=1.4.8

# Necessary tools and help text
TOOLS=(docker gcloud)
HELP="$(basename "$0") [-h|--help] [-t|tools] -- script to build the snaptools image and push to GCR
where:
...
function main(){
    ...
    docker build --no-cache -t "$GCR_URL:$IMAGE_TAG" \
        --build-arg SNAPTOOLS_VERSION="$SNAPTOOLS_VERSION" "$DIR"
