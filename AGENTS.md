...
3. **Verification** — `verification/Verify<Pipeline>.wdl` compares each output to truth (tolerantly). Keep pipeline-specific compare tasks **here, not in the shared `verification/VerifyTasks.wdl`**, so editing them doesn't trip every other pipeline's path filter. When a `Verify*` comparison fails on tiny float or "1-of-N" differences, it is usually known run-to-run nondeterminism, not a regression — compare with a tolerance by extending an existing tolerant task (e.g. `CompareGeneMetricsWithTolerance`, or the tolerant `CompareSAMs` in `VerifyRNAWithUMIs.wdl`) rather than adding a strict check. **Setting a nondeterminism tolerance:** once a human contributor confirms an observed drift is acceptable (not a real regression), set the threshold to **the largest run-to-run drift observed** for that metric (1×) — measured across several calibration runs so the maximum is a real ceiling, not a single sample. Do **not** pad it with a 2× (or any) safety multiplier; the extra margin only erodes regression sensitivity. If a later honest run legitimately exceeds the threshold, re-measure and raise it to the new observed maximum rather than pre-inflating. Any metric absent from a percentage-threshold table (e.g. `CompareAtacLibraryMetrics` in `VerifyTasks.wdl`) defaults to **exact match** — a newly-nondeterministic metric must be added explicitly or it fails on the first sub-unit drift. When the threshold is a **minimum** (e.g. `CompareScanviH5ad`'s proportion-correlation floor), set it below the worst observed across **all** variants — the GEX and ATAC label paths drift differently (observed ~0.875 GEX vs ~0.783 ATAC), so a floor tuned to one flakes the other.
