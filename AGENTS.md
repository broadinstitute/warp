...
6. **Add/rename/remove a pipeline input → thread it everywhere** — any change to a pipeline's input surface (**including adding** an input, not just rename/remove) requires auditing `pipelines/wdl/<name>/example_inputs/*.json` and `test_inputs/**/*.json` **and** threading the change through the `verification/test-wdls/Test<Name>.wdl` wrapper (declare it and forward it into the pipeline call). None of this is checked by womtool: renamed/removed inputs break silently, and an added top-level input that a test JSON sets but the wrapper omits fails at CI input-binding time (`Extra inputs: ...`). See [Test inputs](#test-inputs) and [Registering a CI test](#registering-a-ci-test-for-a-pipeline-plumbing--scientific) step 2.
