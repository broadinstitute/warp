...
# Read fastq_1, fastq_2, run BWA MEM for alignment, then stream to MergeBamAlignment with unmapped BAM
...
# We need to adjust the disk_multiplier
...
task SamSplitter {
  ...
  output {
    Array[File] split_bams = glob("output_dir/*~{input_bam_suffix}")
  }
  runtime {
    docker: "us.gcr.io/broad-gotc-prod/samtools-picard-bwa:1.0.2-0.7.15-2.26.10-1643840748"
    preemptible: preemptible_tries
