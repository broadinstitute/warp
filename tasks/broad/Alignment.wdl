...
task SamToFastqAndBwaMemAndMba {
  ...
  command <<<
    ...
    if [ -s ~{reference_fasta.ref_alt} ] || ~{allow_empty_ref_alt}; then
      java -Xms1000m -Xmx1000m -jar /usr/local/bin/picard.jar \
        SamToFastq \
        INPUT=~{input_bam} \
        FASTQ=/dev/stdout \
        INTERLEAVE=true \
        NON_PF=true | \
      /usr/gitc/temp/bwa-mem2/bwa-mem2 ~{bwa_commandline} /dev/stdin - 2> >(tee ~{output_bam_basename}.bwa.stderr.log >&2) | \
