...
#  }
  ...
  # Skip alignment step for pre-aligned bams
  String aligned_bam_basename = basename(aligned_bam, aligned_bam_suffix)
  Float mapped_bam_size = size(aligned_bam, "GiB")

  # QC the aligned BAM
  call QC.CollectQualityYieldMetrics as CollectQualityYieldMetrics {
    input:
      input_bam = aligned_bam,
      metrics_filename = aligned_bam_basename + ".unmapped.quality_yield_metrics",
      preemptible_tries = papi_settings.preemptible_tries
  }
  ...
  call Processing.SortSam as SortSampleBam {
    input:
      ...
      output_bam_basename = aligned_bam_basename + ".aligned.duplicate_marked.sorted",
      compression_level = compression_level,
      preemptible_tries = if data_too_large_for_preemptibles then 0 else papi_settings.agg_preemptible_tries
  }
  ...
  call Processing.CheckContamination as CheckContamination {
    input:
      ...
      output_prefix = aligned_bam_basename + ".preBqsr",
      preemptible_tries = papi_settings.agg_preemptible_tries,
  ...
  if (perform_bqsr) {
    scatter (subgroup in CreateSequenceGroupingTSV.sequence_grouping) {
      ...
      call Processing.BaseRecalibrator as BaseRecalibrator {
        input:
          input_bam = SortSampleBam.output_bam,
          input_bam_index = SortSampleBam.output_bam_index,
          recalibration_report_filename = aligned_bam_basename + ".recal_data.csv",
    ...
    call Processing.GatherBqsrReports as GatherBqsrReports {
      input:
        ...
        output_report_filename = aligned_bam_basename + ".recal_data.csv",
        preemptible_tries = papi_settings.preemptible_tries
    }
