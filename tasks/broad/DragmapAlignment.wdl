...
task SamToFastqAndDragmapAndMba {
  ...
  command <<<
    ...
    dragen-os -b ~{input_bam} -r dragen_reference --interleaved=1 --preserve-map-align-order 2> >(tee ~{output_bam_basename}.dragmap.stderr.log >&2) | samtools view -h -O BAM - > aligned.bam
