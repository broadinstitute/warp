...
task TerraCopyFilesFromCloudToCloud {
  ...
  command {
    ...
    if ! grep -q no_contamination contamination; then
      gcloud storage cp --project=warp-pipeline-dev --billing-project=warp-pipeline-dev -L cp.log contamination ~{destination_cloud_path}.contamination
    fi
    gcloud storage cp --project=warp-pipeline-dev --billing-project=warp-pipeline-dev ~{sep=' ' files_to_copy} ~{destination_cloud_path}
  }
