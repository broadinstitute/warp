...
task Phase {
  input {
    ...
    String beagle_docker = "us.gcr.io/broad-dsde-methods/jsotobroad/beagle:beagle.28Jul25.224.jar"
    ...
    Int disk_size_gb = ceil((3 * size(dataset_vcf, "GiB")) + size(ref_panel_bref3, "GiB")) + 10          # value may need to be adjusted
  ...
  command <<<
    ...
    -jar /usr/gitc/beagle.28Jul25.224.jar \
    gt=~{dataset_vcf} \
    ref=~{ref_panel_bref3} \
...
task Impute {
  input {
    ...
    Int cpu = 8                    # This parameter is used as the nthreads input to Beagle which is part of how we make it determinstic.  Changing this value may change the output generated by the tool
    Int memory_mb = 32000          # value depends on chunk size, the number of samples in ref and target panel, and whether imputation is performed
    Int xmx_mb = memory_mb - 4000             # I suggest setting this parameter to be 85-90% of the memory_mb parameter
    Int disk_size_gb = ceil((3 * size(dataset_vcf, "GiB")) + size(ref_panel_bref3, "GiB")) + 10         # value may need to be adjusted
  }
