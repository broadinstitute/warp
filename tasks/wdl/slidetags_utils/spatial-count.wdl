...
task count {
  ...
  command <<<
    ...
    for fastq in "${fastqs[@]}" ; do
        if ! gsutil --gcs-project-for-requester-pays warp-pipeline-dev stat "$fastq" &> /dev/null ; then
    ...
    for puck in "${pucks[@]}" ; do
        if ! gsutil --gcs-project-for-requester-pays warp-pipeline-dev stat "$puck" &> /dev/null ; then
