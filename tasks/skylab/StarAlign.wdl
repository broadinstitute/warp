...
task MergeStarOutput {
  ...
  input {
    ...
    Int machine_mem_mb =  if( (ceil((size(matrix, "Gi") + size(features, "Gi") + size(barcodes, "Gi")) * 2) + 10) > 100) then 100
                          else (ceil((size(matrix, "Gi") + size(features, "Gi") + size(barcodes, "Gi")) * 2) + 10)
    Int cpu = 1
