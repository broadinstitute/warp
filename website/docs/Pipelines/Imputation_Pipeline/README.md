...
Initial tests of both the Imputation and subsequent Polygenic Risk Score pipelines revealed that disease risk scores were lower when computed from imputed array data as opposed to whole-genome sequencing data, possibly because of incorrectly genotyped sites in the 1000 Genomes (1000G) reference. As a result, the 1000G reference files were modified for the Imputation pipeline as described below. You can view the original, unmodified 1000G VCFs [here](https://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/). 

To remove the candidate "bad" sites from the 1000G reference panel, allele frequencies were compared between it and the GnomadV2 reference panel. First, the Broad Methods team ran the [BuildAFComparisonTable workflow](https://github.com/broadinstitute/warp/tree/develop/scripts/BuildAFComparisonTable.wdl) to create a table of the allele frequencies for both reference panels. Then, they applied the [FilterAFComparisonTable workflow](https://github.com/broadinstitute/warp/tree/develop/scripts/FilterAFComparisonTable.wdl) to compare the observed number of alleles in 1000G to the expected number of alleles set by the GnomadV2 reference using a two-sided binomial p-value. If both p-values were less than 1e-10, then the site was flagged as "bad." Lastly, they used the [RemoveBadSitesById workflow](https://github.com/broadinstitute/warp/tree/develop/scripts/RemoveBadSitesById.wdl) to remove the "bad" sites. Read more about the comparison in the [technical report](https://github.com/broadinstitute/hydro.gen/tree/main/Analysis/cleaned_1000G_for_imputation). 

For questions regarding reference generation, email [Chris Kachullis](mailto:ckachuli@broadinstitute.org). 
