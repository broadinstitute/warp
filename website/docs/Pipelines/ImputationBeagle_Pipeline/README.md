...
|:------------------------------------------------------------------------------------------------------------------:|:------------:|:----------------------------------:|:------------------------------------------------------------------------------:|
| [ImputationBeagle_v2.3.0](https://github.com/broadinstitute/warp/releases?q=ImputationBeagle_v2.3.0&expanded=true) | November, 2025 | Terra Scientific Pipeline Services | Please [file an issue in WARP](https://github.com/broadinstitute/warp/issues). |
...
| ExtractUniqueVariantIds              | query                                                     | bcftools                           | Extracts unique variant IDs from input and reference panel VCFs in CHROM:POS:REF:ALT format for use in variant counting                                                                                                                                                                                                          |
| CountVariantsInChunks                | comm                                                      | bash                               | Counts variants in the filtered VCF chunk using unique variant ID lists; Returns the number of variants in the chunk and variants also present in the reference panel.                                                                                                                                                            |
| CountVariantsInChromosome            | grep, wc                                                  | bash                               | Counts unique variants for a given chromosome from the unique variant ID list generated from the raw input VCF                                                                                                                                                                                                                     |
...
| StoreChunksInfo                      |                                                           | Python (pandas)                    | Gathers all results from CheckChunks; creates chunk-level and contig-level (chromosome-level) metrics files with variant counts                                                                                                                                                                                                   |
...
| chunks_info                   | TSV from StoreChunksInfo task; contains the chunk intervals, variant counts per chunk (filtered input and overlap with reference panel), and whether each chunk was successfully imputed.                                  | TSV   |
| contigs_info                  | TSV from StoreChunksInfo task; contains contig-level (chromosome-level) aggregated metrics including total variant counts in the raw input, filtered input, and overlap with reference panel for each processed chromosome. | TSV   |
