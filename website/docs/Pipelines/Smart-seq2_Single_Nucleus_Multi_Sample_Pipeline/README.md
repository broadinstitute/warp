...
The pipeline is designed to process snRNA-seq data from multiple cells. Overall, the workflow trims paired-end FASTQ files, aligns reads to the genome using a modified GTF, [counts intronic and exonic reads](#creating-the-cell-by-gene-matrix-loom), and calculates quality control metrics.
