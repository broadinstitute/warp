...
declare -r WARP_ROOT=$(cd $(dirname ${SCRIPT_DIR}) && pwd)
...
function deploy_options() {
  ...
  elif [[ "${wdl_dir}" =~ .*"skylab".* ]] && [[ -f ${WARP_ROOT}/tests/skylab/test.options.json ]]; then
    base_options=${WARP_ROOT}/tests/skylab/test.options.json
  else
