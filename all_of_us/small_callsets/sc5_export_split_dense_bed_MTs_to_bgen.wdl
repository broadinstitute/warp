...
# Workflow: sc5_export_split_dense_MTs_to_bgen
...
workflow export_split_dense_MTs_to_bgen {
    input {
        # Analysis Parameters
    ...
    call export_split_dense_MTs_to_bgen {
    ...
    output {
        File exome_manifest = export_split_dense_MTs_to_bgen.exome_manifest
    }
}

task export_split_dense_MTs_to_bgen{
    input {
        ...
        try:
            ...
            for cluster in cluster_client.list_clusters(request={"project_id": "~{gcs_project}", "region": "~{region}"}):
                if cluster.cluster_name == cluster_name:
                    ...
                    staging_cmd = f'gsutil cp -r ~{output_gs_url}/~{version_label}/~{task_label}/exome/bgen/manifest.bgen.txt  ~{version_label}.exome.manifest.bgen.txt'
                    print(staging_cmd)
    ...
    output {
        File exome_manifest = "~{version_label}.exome.manifest.bgen.txt"
