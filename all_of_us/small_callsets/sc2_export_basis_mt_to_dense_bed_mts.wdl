...
# Workflow: sc2_export_basis_MT_to_bed_dense_MTs
# Summary: Given a basis Hail MatrixTable created from VDS and annotated by bed names,
# create corresponding dense Hail MatrixTables for each bed file.
...
##
## This WDL pipeline processes a basis MatrixTable annotated by bed names, to create the corresponding dense Hail
## MatrixTable for each bed file.
...
workflow export_basis_MT_to_bed_dense_MTs {
    input {
        # Analysis Parameters
        String input_basis_dense_mt_json_path  # path of the basis dense MatrixTable json file, output from workflow sc1 vds2mt
        File exome_bed_path  # Path to the exome BED file
        File clinvar_bed_path  # Path to the ClinVar BED file
        File acaf_threshold_bed_path  # Path to the ACAF threshold BED file
        ...
        String output_dense_mts_json_file_name # File name of the bed dense MatrixTables json file, suffix should be .json
    ...
    call export_basis_MT_to_bed_dense_MTs {
        ...
        input:
            input_basis_dense_mt_json_path = input_basis_dense_mt_json_path,
            ...
            output_dense_mts_json_file_name = output_dense_mts_json_file_name
    }

    output {
        File dense_mts_json_file = export_basis_MT_to_bed_dense_MTs.dense_mts_json_file
    }
}

task export_basis_MT_to_bed_dense_MTs {
    input {
        # Task-specific inputs with descriptions
        String input_basis_dense_mt_json_path
        File exome_bed_path
        ...
        String hail_docker
        String output_dense_mts_json_file_name
    }
            ...
            for cluster in cluster_client.list_clusters(request={"project_id": "~{gcs_project}", "region": "~{region}"}):
                if cluster.cluster_name == cluster_name:
                    ...
                    submit_cmd = f"""
                        ...
                        --acaf_threshold_bed_path ~{acaf_threshold_bed_path}
                        --version_label ~{version_label} --input_basis_dense_mt_json_path ~{input_basis_dense_mt_json_path}
                        --task_label ~{task_label}
                        --output_dense_mts_json_file_name ~{output_dense_mts_json_file_name}
                        --output_gs_url ~{output_gs_url}
                    """
                    ...
                    staging_cmd = f'gsutil cp -r ~{output_gs_url}/~{version_label}.~{task_label}.~{output_dense_mts_json_file_name}  ~{version_label}.~{task_label}.~{output_dense_mts_json_file_name}'
                    print(staging_cmd)
    ...
    output {
        File dense_mts_json_file = "~{version_label}.~{task_label}.~{output_dense_mts_json_file_name}"
