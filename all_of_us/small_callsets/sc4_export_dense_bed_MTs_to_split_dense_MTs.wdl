...
# Workflow: sc4_export_dense_bed_MTs_to_split_dense_MTs
# Summary: Given a dense MatrixTable with multi-allelic variants for each bed name,
# create corresponding split dense Hail MatrixTables for each bed file.
...
##
## This WDL pipeline processes a list of dense MatrixTable with multi-allelic variants for each bed file, to create the corresponding dense Hail
...
workflow export_dense_bed_MTs_to_split_dense_MTs {
    input {
        # Analysis Parameters
        ...
        File input_dense_mts_json_path # File name of the bed dense MatrixTables json file, suffix should be .json
        String output_split_dense_mts_json_file_name # File name for the split bed dense MatrixTables json file, suffix should be .json
    ...
    call export_dense_bed_MTs_to_split_dense_MTs {
        ...
        input:
            ...
            input_dense_mts_json_path = input_dense_mts_json_path,
            output_split_dense_mts_json_file_name = output_split_dense_mts_json_file_name
                                           }

    output {
        File split_dense_mts_json_file = export_dense_bed_MTs_to_split_dense_MTs.split_dense_mts_json_file
    }
}

task export_dense_bed_MTs_to_split_dense_MTs {
    input {
        ...
        String hail_docker
        File input_dense_mts_json_path
        String output_split_dense_mts_json_file_name
    }
        ...
        try:
            ...
            for cluster in cluster_client.list_clusters(request={"project_id": "~{gcs_project}", "region": "~{region}"}):
                if cluster.cluster_name == cluster_name:
                    ...
                    staging_cmd = f'gsutil cp -r ~{output_gs_url}/~{version_label}/~{task_label}/~{version_label}.~{task_label}.~{output_split_dense_mts_json_file_name}  ~{version_label}.~{task_label}.~{output_split_dense_mts_json_file_name}'
                    print(staging_cmd)
    ...
    output {
        File split_dense_mts_json_file = "~{version_label}.~{task_label}.~{output_split_dense_mts_json_file_name}"
