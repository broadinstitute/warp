...
task Demultiplexing {
  ...
  command <<<
    ...
    declare -a r1_files=(~{sep=' ' fastq_input_read1})
    declare -a r2_files=(~{sep=' ' fastq_input_read2})

    #for every file in the array r1_files, move to working dir
    for file in "${r1_files[@]}"; do
      mv $file $working_directory
      echo "Moved $file to working directory"
    done

    #for every file inarray r2_files move to working directory
    ...
    stats_file_path = '~{plate_id}.stats.txt'
    adapter_counts = {}
...
task Hisat_paired_end {
    ...
    command <<<
        ...
        task() {
          ...
          start=$(date +%s)
          echo "Run sort r1"
          zcat $working_directory/batch*/"$r1_file" | paste - - - - | sort -k1,1 -t " " | tr "\t" "\n" > "${sample_id}-R1_sorted.fq"
          end=$(date +%s)
          elapsed=$((end - start))
          ...
          start=$(date +%s)
          echo "Run sort r2"
          zcat $working_directory/batch*/"$r2_file" | paste - - - - | sort -k1,1 -t " " | tr "\t" "\n" > "${sample_id}-R2_sorted.fq"
          end=$(date +%s)
          elapsed=$((end - start))
          ...
          start=$(date +%s)
          echo "Run hisat"
          hisat-3n $working_directory/$genome_fa_basename \
          -q \
          ...
          rm $working_directory/batch*/${sample_id}-R1.fq.gz $working_directory/batch*/${sample_id}-R2.fq.gz
      ...
      # define lists of r1 and r2 fq files
      R1_files=($(ls $working_directory/batch*/ | grep "\-R1.fq.gz"))
      R2_files=($(ls $working_directory/batch*/ | grep "\-R2.fq.gz"))
