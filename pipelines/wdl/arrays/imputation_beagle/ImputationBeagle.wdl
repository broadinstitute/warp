...
workflow ImputationBeagle {
  ...
  scatter (contig_index in range(length(contigs_to_process))) {
    ...
    call beagleTasks.GatherVcfsNoIndex as GatherContigVcfs {
      input:
  ...
  call beagleTasks.GatherVcfsNoIndex as GatherVcfs {
    input:
      input_vcfs = GatherContigVcfs.output_vcf,
  ...
  call beagleTasks.CreateVcfIndex as CreateIndexForGatheredVcf {
    input:
      vcf_input = GatherVcfs.output_vcf,
      gatk_docker = gatk_docker,
      preemptible = 0
  }
