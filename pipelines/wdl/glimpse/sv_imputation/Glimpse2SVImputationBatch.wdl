    ...
    scatter (chromosome in chromosomes) {
        ...
        if (info_filter_for_inclusion > 0.0) {
            call FilterVcfByInfo as FilterBubblePosteriorsByInfo {
                input:
                    vcf = UpdateHeader.output_bcf,
                    vcf_idx = UpdateHeader.output_bcf_index,
        ...
        File filtered_bubble_posteriors_vcf = select_first([FilterBubblePosteriorsByInfo.output_vcf, UpdateHeader.output_bcf])
        File filtered_bubble_posteriors_vcf_idx = select_first([FilterBubblePosteriorsByInfo.output_vcf_idx, UpdateHeader.output_bcf_index])
