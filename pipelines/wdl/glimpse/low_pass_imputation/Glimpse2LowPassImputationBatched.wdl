...
import "./Glimpse2LowPassImputation.wdl" as Glimpse2LowPassImputationSubworkflow
...
workflow Glimpse2LowPassImputationBatched {
    ...
    if (defined(crams)) {
        call Glimpse2LowPassImputationSubworkflow.SplitIntoBatches as SplitIntoSampleBatches {
            input:
        ...
        scatter(batch_idx in range(length(SplitIntoSampleBatches.crams_batches))) {
            ...
            call Glimpse2LowPassImputationSubworkflow.Glimpse2LowPassImputation as RunBatch {
                input:
                    contigs = contigs,
        ...
        scatter(contig_idx in range(length(contigs))) {
            ...
            call Glimpse2LowPassImputationSubworkflow.SelectVariantRecordsOnly as SelectContigVariants {
                input:
                    vcf = RecomputeAndAnnotate.merged_imputed_vcf,
                    basename = output_basename + "." + contigs[contig_idx] + ".imputed.merged.only_variants"
            }

            call Glimpse2LowPassImputationSubworkflow.CreateHomRefSitesOnlyVcf as CreateContigHomRefVcf {
                input:
                    vcf = RecomputeAndAnnotate.merged_imputed_vcf,
        ...
        if (length(batched_coverage_metrics) > 0) {
            call Glimpse2LowPassImputationSubworkflow.CombineCoverageMetrics as CombineBatchCoverageMetrics {
                input:
    ...
    if (!defined(crams)) {
        call Glimpse2LowPassImputationSubworkflow.Glimpse2LowPassImputation as RunSingle {
            input:
                contigs = contigs,
                reference_panel_prefix = reference_panel_prefix,
                input_vcf = input_vcf,
    ...
    call Glimpse2LowPassImputationSubworkflow.GatherVcfsNoIndex {
        input:
    ...
    call Glimpse2LowPassImputationSubworkflow.CreateVcfIndexAndMd5 {
        input:
    ...
    call Glimpse2LowPassImputationSubworkflow.GatherVcfsNoIndex as GatherVcfsNoIndexHomRefOnly {
        input:
    ...
    call Glimpse2LowPassImputationSubworkflow.CreateVcfIndexAndMd5 as CreateVcfIndexAndMd5HomRefOnly {
        input:
    ...
    call Glimpse2LowPassImputationSubworkflow.CollectQCMetrics {
        input:
