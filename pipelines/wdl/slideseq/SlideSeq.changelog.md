...
* Replaced `Boolean? count_exons` in `STARsoloFastqSlideSeq` with `String exon_solo_directory`; the workflow now passes a pre-computed directory path instead of a boolean, eliminating boolean logic from the task body. Pipeline outputs are unchanged.
